...
for i, conf in enumerate(conf_unique):
	...
	ax1[i].scatter(wide_arr, ratio_fit_arr, s=25, ec='C0', fc='C0', label='fit')
	...
	ax2[i].scatter(wide_arr, sum_fit_obs, s=25, ec='C0', fc='C0', label='fit - obs')

	ax2[i].scatter(wide_arr, sum_sim, s=25, ec='C1', fc='none', marker='s', label='sum - sim')
	ax2[i].scatter(wide_arr, sum_fit_sim, s=25, ec='C1', fc='C1', marker='s', label='fit - sim')
...
ax1[0].legend(loc='lower left', fontsize=8)
