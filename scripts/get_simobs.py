...
for i in range (0,10):
    for j in range (0,15):
        ...
        myproject_dir = './projects/'

        myproject = myproject_dir+configuration[i]+'_'+str('%.1f'%(source_size))+'mrs0_project'
        myskymodel = myskymodel_dir+configuration[i]+'_'+'ref_image'+str('%.1f'%(source_size))+'mrs0'
        
        # importfits(fitsimage=myproject+'.fits', 
        #             imagename=myskymodel, 
        #             overwrite=True)
